...
def construct_prompts(reference, pseudocode):
    ...
    user_prompt = (
        ...
        "\n- CRITICAL BYZANTINE EQUIVOCATION LOGIC: Read the PROPOSE/ECHO behavior in the pseudocode carefully. IF the pseudocode is Authenticated Echo Broadcast(AEB) or Authenticated Double-Echo Broadcast(ADB) or Optimistic reliable broadcast, then ignore this BYZANTINE EQUIVOCATION LOGIC and remain consistent with `draft.md`."
...
def main():
    ...
    name = "BrachaRBC"  # change here

    base_dir = os.path.dirname(__file__)
    pseudocode_filename = os.path.join(base_dir, f"{name}.txt")
    json_file = os.path.join(base_dir, "Structured_Template", f"{name}.json")
    pseudocode = read_file_content(pseudocode_filename)
    system_prompt, user_prompt = construct_prompts(reference, pseudocode)

    messages_step1 = [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}]
